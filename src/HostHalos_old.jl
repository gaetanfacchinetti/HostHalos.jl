...
import CosmoTools: Halo, HaloProfile, nfwProfile, αβγProfile, halo_from_ρs_and_rs, m_halo, ρ_halo, μ_halo, coreProfile
...
function age_host(z::Real = 0, ::Type{T} = MM17Gamma1, cosmo::BkgCosmology = planck18_bkg; kws...) where {T<:MM17Model}
    z_max = lookback_redshift((1e+4 * MYR_TO_S))
...
ρ_baryons_spherical(r::Real, ::Type{T} = MM17Gamma1) where {T<:HostModel} = QuadGK.quadgk(xp -> der_ρ_baryons_spherical(xp, r, T), 0, 1, rtol=1e-4)[1] 
m_baryons_spherical(r::Real, ::Type{T} = MM17Gamma1) where {T<:HostModel} = 4.0 * π * QuadGK.quadgk(rp -> rp^2 * ρ_baryons_spherical(rp, T), 0, r, rtol=1e-3)[1] 
...
μ_baryons_spherical(x::Real, ::Type{T}) where {T<:DMOnlyMM17Model} = 0.0
...
circular_velocity(r::Real, ::Type{T} = MM17Gamma1) where {T<:HostModel} = sqrt(G_NEWTON * m_host_spherical(r, T) / r) 

""" circular velocity in (km / s) for `r` in Mpc """
circular_velcoity_kms(r::Real, ::Type{T} = MM17Gamma1) where {T<:HostModel} = circular_velocity(r, T) *  MPC_TO_KM

""" circular period in s for `r` in Mpc """
circular_period(r::Real, ::Type{T} = MM17Gamma1) where {T<:HostModel} = 2.0 * π * r / circular_velocity(r, T)  

""" number or circular orbits with `r` in Mpc """
number_circular_orbits(r::Real, z::Real = 0, ::Type{T} = MM17Gamma1, cosmo::BkgCosmology = planck18_bkg; kws...) where {T<:HostModel} = floor(Int, age_host(z, T, cosmo, kws...) / circular_period(r, T))

""" Jeans dispersion in (Mpc / s)"""
velocity_dispersion_spherical(r::Real, ::Type{T} = MM17Gamma1) where {T<:HostModel} = sqrt(G_NEWTON / ρ_dm(r, T) * QuadGK.quadgk(rp -> ρ_dm(rp, T) * m_host_spherical(rp, T)/rp^2, r, tidal_radius(T), rtol=1e-3)[1])

""" Jeans dispersion in (km / s)"""
velocity_dispersion_spherical_kms(r::Real, ::Type{T} = MM17Gamma1) where {T<:HostModel} = velocity_dispersion_spherical(r, T) * MPC_TO_KM
